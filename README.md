...
| Путь | Назначение |
|-------------------|-------------------------------------------------------------------------------------------------------------------------|
| [secrets/](secrets/) | Сервисный аккаунт Google Drive (drive_sa.json) - необходимо добавить |
...
### Инструкция получения файла drive-sa.json
Войти в Google Cloud Console
- Перейти на https://console.cloud.google.com/
- Авторизоваться в аккаунте Google.
- В правом верхнем углу выбрать или создать новый проект:
  - Нажать на селектор проектов.
  - “Создать проект” > указать имя, выбрать организацию.

Включить Google Drive API
- Внутри выбранного проекта перейти в:
...
- Заполнить:
  - Имя аккаунта: airflow-uploader
  - Описание: (по желанию)
  - Нажать “Создать и продолжить”.
  - Права не добавлять — просто нажать “Готово”.

Сгенерировать JSON-ключ drive_sa.json
- В списке сервисных аккаунтов:
  - Найти созданный (airflow-uploader), нажми три точки справа > “Управление ключами”.
  - Нажать: Добавить ключ → Создать новый ключ > JSON → Создать.
  - Файл *.json скачается автоматически.
- Переименовать в drive_sa.json и скопировать в [secrets/](secrets/)

Выдать доступ к нужной папке на Google Drive
Сервисный аккаунт не имеет доступа к Google Drive по умолчанию.
- Перейти на https://drive.google.com/
- Создать папку (например: airflow-uploads)
- Нажать правой кнопкой > “Открыть доступ”
- Вставить email сервисного аккаунта (он заканчивается на @<project>.iam.gserviceaccount.com)
  - Выдать права: Редактор
  - Нажать “Отправить”

Получить GDRIVE_FOLDER_ID
- Открыть нужную папку на Google Drive.
- Скопировать ID из URL:
```
https://drive.google.com/drive/folders/**5kN_kULkhjhOIuiilksON24Az07b3--lUdkH**
```
Использовать в .env:
```
GDRIVE_FOLDER_ID=5kN_kULkhjhOIuiilksON24Az07b3--lUdkH
```
